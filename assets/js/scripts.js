/*!
 * Theme Name: Bitcoin Robot
 * Author: Indigo
 * Author URI: http://indigo.com
 * Version   :  1.0.0
 * Updated   :  12/10/2022
**/
var NioApp = (function (jQ, win, doc){
    "use strict";
    var NioApp = {AppInfo: {name: "Indigo", package: "1.0.0", version: "1.0.0", author: "Indigo"} },
        ...
        if ($particles_bg.exists()) {
            $particles_bg.each(function(){
                var $self = $(this), $self_id = $self.attr('id'),
                    ...
                    _pt_shape_op    = ($self.data('pt-shape-op')) ? parseFloat($self.data('pt-shape-op')) : 1, 
                    _pt_line        = ($self.data('pt-line')) ? $self.data('pt-line') : '#fff',
                    _pt_line_op     = ($self.data('pt-line-op')) ? parseFloat($self.data('pt-line-op')) : 1;
